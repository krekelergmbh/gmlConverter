...
def create_tab_terrain(notebook):
    ...
    ttkb.Label(tab,
               text="Dort DGM1-Kacheln als XYZ- oder ASC-Datei herunterladen "
                    "(gzip .gz wird direkt unterstützt).",
    ...
    ttkb.Label(tab, text="Tipp: DGM-Kacheln direkt in die Liste ziehen.",
               font=("Segoe UI", 10), foreground=MUTED)\
    ...
    def _prepare(gml_path):
        ...
        try:
            mesh, _ = dgm.prepare_terrain(
                dgm_file_list, gml_path=gml_path or None, log=print)
            tab.update()
            return mesh
    ...
    ttkb.Button(btn_row, text="Als IFC exportieren", style="CTA.TButton",
                command=export_ifc).pack(side="left")
    ttkb.Button(btn_row, text="Vorschau", style="Grey.TButton",
                command=show_preview).pack(side="left", padx=(10, 0))
